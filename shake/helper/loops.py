...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill in ['abound']:
            preact = True
        feat_s, logit_s,_,_ = model_s(input, is_feat=True, preact=preact)
        #feat_s, logit_s,_,_ = model_s(mixed_inputs, is_feat=True, preact=preact)
        with torch.no_grad():
            feat_t, logit_t, weight, bias = model_t(input, is_feat=True, preact=preact)
            #feat_t, logit_t, weight, bias = model_t(mixed_inputs, is_feat=True, preact=preact)
