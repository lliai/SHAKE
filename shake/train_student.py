...
from models.util import Connector, Translator, Paraphraser, Shake
...
def main():
    ...
    model_t.eval()
    model_s.eval()
    feat_t, _ ,_,_= model_t(data, is_feat=True)
    feat_s, _ ,_,_= model_s(data, is_feat=True)
    ...
    if opt.distill == 'kd':
        ...
        Shake_t = Shake(feat_t)
        module_list.append(Shake_t)
        trainable_list.append(Shake_t)
        criterion_kd = DistillKL(opt.kd_T)
